...
import co.codingnomads.kraken.model.trade.pojo.KrakenOpenPositions;
 ...
 * by the key (position_txid) and the value (KrakenOpenPositions) corresponding to call's output.
 *
...
public class GetOpenPositionsOutput extends OutputWrapper<Map<String, KrakenOpenPositions>> {
     ...
     * and corresponding body with associated information to the KrakenOpenPositions POJO
     *
    ...
    public GetOpenPositionsOutput(@JsonProperty("error") String[] error,
                                  @JsonProperty("result") Map<String, KrakenOpenPositions> result) {
        super(result, error);
    }
}
